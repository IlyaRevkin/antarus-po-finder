...
class SettingsPage(QWidget):
    ...
    def _build_general_tab(self) -> QWidget:
        ...
        # ── Inspection folder ──────────────────────────────────────────────────
        insp_box = QGroupBox('Папка осмотра (фото/сканы)')
        insp_layout = QHBoxLayout(insp_box)
        self._insp_path_input = QLineEdit()
        self._insp_path_input.setFixedHeight(34)
        self._insp_path_input.setPlaceholderText('По умолчанию: локальная папка firmware')
        insp_layout.addWidget(self._insp_path_input, 1)
        ...
        insp_layout.addWidget(browse_insp_btn)
        ...
        insp_layout.addWidget(save_insp_btn)
        layout.addWidget(insp_box)

        # ── Sync interval ──────────────────────────────────────────────────────
        sync_box = QGroupBox('Интервал синхронизации с диском')
        sync_layout = QHBoxLayout(sync_box)
        sync_layout.addWidget(QLabel('Каждые'))
        self._sync_interval_input = QLineEdit()
        self._sync_interval_input.setFixedWidth(60)
        self._sync_interval_input.setFixedHeight(34)
        sync_layout.addWidget(self._sync_interval_input)
        sync_layout.addWidget(QLabel('минут'))
        ...
        sync_layout.addWidget(save_sync_btn)
